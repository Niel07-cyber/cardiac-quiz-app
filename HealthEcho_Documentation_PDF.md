...
subtitle: "Technical Documentation and Project Report"
...
header-includes:
  - \usepackage{fancyhdr}
  - \pagestyle{fancy}
  - \fancyhead[L]{HealthEcho Documentation}
...
The HealthEcho system follows a modern three-tier architecture:
...
- Progress bar
- Two-column layout:
  - Left: Video player with speed controls
  - Right: Question and answer options
- Feedback messages
- Navigation buttons
...
1. **Complete Quiz Flow**: Start quiz → Load questions → Watch videos → Select answers → View results
2. **Error Scenarios**: Network disconnection, missing video files, invalid API responses
3. **Browser Compatibility**: Video playback and UI responsiveness across browsers
